...
namespace all_tests
{
	...
	public:
		...
		TEST_METHOD(test_03a)
		{
			...
			std::transform(v.cbegin(), v.cend(), v.begin(), [](int i) { return pow(i, 3); });
			Assert::AreEqual(3ull, v.size());
			Assert::AreEqual(1, v[0]);
		...
		TEST_METHOD(test_03b)
		{
			...
			std::transform(std::begin(x), std::end(x), y.begin(), std::back_inserter(d), [](int x, int y) {return sqrt(x * x + y * y); });
		...
		TEST_METHOD(test_04c)
		{
			...
			auto total_age = std::accumulate(v.begin(), v.end(), 0, [](int age, person const& p) {return age + p.age; }); // TODO: sum of all ages
		...
		TEST_METHOD(test_05a)
		{
			...
			auto number_of_negative = std::count_if(v.begin(), v.end(), [](int i) { return i < 0; }); // TODO: 
		...
		TEST_METHOD(test_05c)
		{
			...
			auto number_in_first_quadrant = std::count_if(v.begin(), v.end(), [](point const& p) { return p.x > 0 && p.y > 0; }); // TODO: 
